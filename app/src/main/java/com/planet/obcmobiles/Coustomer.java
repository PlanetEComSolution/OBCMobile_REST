...
public class Coustomer extends Activity {
    public String c;
    EditText t1;
    Button b1, home;
    int count;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.ac_status);
        ...
        b1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (c.length() == 0) {
                    Toast.makeText(getApplicationContext(), "Please enter Complaint reference no", Toast.LENGTH_LONG).show();
                } else {
                    //new Perform().execute();
                    fetch_ComplaintStatus_REST_API();
                }
            }
        });
    }
    ...
    public void SuccessAlertDialog() {
       // Log.d("BHANU_1", resp);
        ...
        alertDialogBuilder.setTitle("Your Complaint Status");
        alertDialogBuilder.setMessage(resp).setCancelable(true)
   ...
   /* public void userregisteration_web() {
        ...
        final String SOAP_ACTION = "http://tempuri.org/ComplaintStatus";
        final String METHOD_NAME = "ComplaintStatus";

        SoapObject request = new SoapObject(NAMESPACE, METHOD_NAME);
        System.out.println("ccccccccccccccccccccafter" + c);
        request.addProperty("ComplaintRefNo", c);
        ...
        try {
            ...
            System.out.println(" ttttttttttttttttttttttttttt" + Jsonstring);
            String news = Jsonstring.substring(Jsonstring.indexOf("["));
            String n1 = news;
            jArray = new JSONArray(news);

            for (int k = 0; k < (jArray.length()); k++) {
                JSONObject json_obj = jArray.getJSONObject(k);
                resp = json_obj.getString("ComplaintStatus");
                Log.d("BHANU_F", resp);
            }
        } catch (Exception e) {
            // count++;
    ...
    public void internetAlertDialog() {
        AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(Coustomer.this);
        ...
        try {
           ...
           // Log.d("PIN_CODE", recved);
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                resp = jsonObject.getString("ComplaintStatus");
...
*/
    ...
    private void fetch_ComplaintStatus_REST_API() {
        Progress_Dialog.show_dialog(this);
        HashMap<String, String> hashMap = new HashMap<>();

        hashMap.put("ComplaintRefNo", c);

        API_Interface apiService = API_Client.getClient().create(API_Interface.class);
        apiService.FetchComplaintStatus(hashMap)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Subscriber<List<ComplaintStatusResponse>>() {
                    @Override
                    ...
                    @Override
                    public void onNext(List<ComplaintStatusResponse> responseList) {

                        Progress_Dialog.hide_dialog();

                        if (responseList != null && responseList.size() > 0) {
                            resp = responseList.get(0).getComplaintStatus();

                            SuccessAlertDialog();
                        } else {
                            Toast.makeText(Coustomer.this, "Please enter correct reference no.", Toast.LENGTH_SHORT).show();
                        }
...
/*
    public class Perform extends AsyncTask<Void, Void, Void> {
        private ProgressDialog dialog = new ProgressDialog(Coustomer.this);
